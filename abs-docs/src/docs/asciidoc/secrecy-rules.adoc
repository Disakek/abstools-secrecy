...
*Join:* The join operation selects the *least upper bound* of two secrecy levels.
It ensures the result reflects the highest confidentiality required among them.
...
If there is no secrecy level for an expression we instead assume the lowest possible secrecy level from the given lattice.
