 ...
 */
public class FlowInsensitiveExpVisitor extends SecrecyExpVisitor {
     ...
     */
    public FlowInsensitiveExpVisitor(Model m, HashMap<ASTNode<?>,String> _secrecy, SecrecyLatticeStructure secrecyLatticeStructure, SemanticConditionList errors, LinkedList<ProgramCountNode> programConfidentiality, SecrecyStmtVisitor stmtVisitor, LinkedList<CalledMethod> methodsCallingOthers) {
    ...
    public String visit(NewExp newExp) {
        ...
        if(calledParams.getNumChild() > 0) {
            ...
            for(int i = 0; i < calledParams.getNumChild(); i++) {
                ...
                //Retrieve the secrecy of the input parameters
                String calledSecrecy = this.visit(calledParams.getChild(i));
                //Retrieve the secrecy for the class parameters of the class we try to create an object for                
                String definedSecrecy = _secrecy.get(declaredParams.getChild(i));
                
                if(calledSecrecy == null) { 
                    calledSecrecy = secrecyLatticeStructure.getMinSecrecyLevel();
                }
                ...
                //Ensure that the secrecy of the parameters we call it with is smaller or equal to the stored max secrecy otherwise add a type error
